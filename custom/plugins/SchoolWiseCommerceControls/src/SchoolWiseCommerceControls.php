...
namespace SchoolWiseCommerceControls;
...
use SchoolWiseCommerceControls\Service\CustomFieldsInstaller;

class SchoolWiseCommerceControls extends Plugin
{
    ...
    public function uninstall(UninstallContext $uninstallContext): void
    {
        ...
        $customFieldsInstaller = $this->getCustomFieldsInstaller();

        // Get all custom field set IDs to remove
        $customFieldSetIds = $customFieldsInstaller->getAllCustomFieldSetIds($context);

        if (!empty($customFieldSetIds)) {
            $customFieldsInstaller->deleteCustomFieldSets($customFieldSetIds, $context);
        }
    }
